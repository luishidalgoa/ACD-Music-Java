...
public class ReproductionList_Card {
    ...
    public void initialize() {
        //vamos a darle a img_picture un pequeño rounded para la imagen
        img_picture.setClip(null);


        //ESTO DEBE BORRARSE
        setData(new ReproductionList());
    }

    @FXML
    public void likeClickOn() {
        ...
        if(svg_like.getContent().equals(estado1)) {
    ...
    public void setData(ReproductionList obj){
        //extraemos la imagen de la primera cancion de la lista
        //File img=new File(obj.getSongs().stream().findFirst().get().getPicture());
        //TEST
        File imgTEST= new File(".\\src\\main\\resources\\dev\\iesfranciscodelosrios\\acdmusic\\assets\\pictures\\user\\Luis.png");
        if(imgTEST.exists()) {
            img_picture.setImage(new javafx.scene.image.Image(imgTEST.toURI().toString()));
        }
    }
}
